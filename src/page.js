  ...
  return (
    <>
        <section className='w-screen h-screen parent backdrop-blur-2xl'>
            <button onClick={handleActive} className='absolute z-50 px-5 py-3 rounded-full bg-gradient-to-br from-blue-900 to-yellow-700 bottom-10 right-7'>
                <span className='text-sm text-white'>
                    {active ? `Deactivate Assistant` : `Activate Assistant`}
                </span>
            </button>
            <div className='absolute bottom-0 z-50 right-32'>
                {active ? "" : <img  src={assistant} alt="" className=' h-[100px]' />}
            </div>
                            ...
                            return (
                                <>
                                   <div className='relative'>
                                        <a key={i} target='blank' href={datum.link} className='z-20 flex items-center gap-2 px-10 py-3 my-2 text-white rounded-lg bg-gradient-to-br from-blue-900 to-zinc-700'>{datum.social} <span className='text-xl'>{datum.icon}</span> </a>
                                        {
                                            active ? <div id='sib' className='absolute z-50 px-5 py-2 text-white rounded-r-xl rounded-tl-xl w-fit backdrop-blur-lg bg-gradient-to-tr bg-blue-950/50 -left-32 '>
                                            <div className='relative'>
                                                 <span className='max-w-[50px]'>{datum.info}</span>
                                                 <img  src={assistant} alt="" className='h-[150px] absolute top-0 -left-32'/>
                                            </div>
                                         </div>
                                         : ""
                                        }
